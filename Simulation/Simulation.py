class Simulation:
	...
	def draw(self):
		...
		for objectHandle in self.staticObjects:
			object = self.getObject(objectHandle)
			object.draw()
		for objectHandle in self.moveableObjects:
			object = self.getObject(objectHandle)
			object.draw()
	...
	def update(self, time):
		...
		for objectHandle in self.moveableObjects:
			object = self.getObject(objectHandle)
			object.recalcVelocity()
			object.moveObject(time)
	...
	def getObject(self, handle):
		...
		if self.handleDictionary.has_key(handle):
