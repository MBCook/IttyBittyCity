...
class Video:
	...
	def loadFromPrefs(self):
		...
		self.fov = self.Prefs.getPref("fov")
		self.near = self.Prefs.getPref("near")
		self.far = self.Prefs.getPref("far")
	...
	def prepareVideo(self):
		...
		glClearColor(48.0 / 255.0, 130.0 / 255.0, 54.0 / 255.0, 0.0)
	...
	def setupCamera(self):
		...
		glMatrixMode(GL_PROJECTION)
		glLoadIdentity()
		gluPerspective(self.fov, self.windowWidth / self.windowHeight, self.near, self.far)

		gluLookAt(self.cameraX, self.cameraY, self.cameraZ,
				  self.cameraPointX, self.cameraPointY, self.cameraPointZ,
				  self.cameraUpX, self.cameraUpY, self.cameraUpZ)
	...
	def setupLights(self):
		...
		glLightfv(GL_LIGHT1, GL_POSITION, (self.lightX,
										   self.lightY,
										   self.lightZ,
										   0.0))
		glEnable(GL_LIGHT1)
