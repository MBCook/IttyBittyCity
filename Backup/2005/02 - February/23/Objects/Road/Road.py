...
class Road(SimulationObject):
	...
	def draw(self):
		...
		glScale(1.0 / 12.0, 1.0 / 12.0, 1.0)

		glTranslatef(self.xPosition + wo, self.yPosition + ho - self.height, self.zPosition)
	...
	def setSize(self, w, h):
		...
		if (self.roadType >= 8) and (self.roadType <= 11):
			...
			tw = 0.50
			th = 0.50
		elif (self.roadType >= 3) and (self.roadType <= 6):
			tsx = 0.25
			tsy = 0.0
		else:
			tsy = 0.0
		...
		VideoRoutines.drawFromArray(arrayData, True)
